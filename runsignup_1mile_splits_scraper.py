"""
runsignup_1mile_splits_scraper.py

Goal:
  Build a CSV dataset of real race results with:
    - 1 Mile finish time
    - Mile 1 split time (when available)
...
Data sources (RunSignup API):
  ...
  - Race details (to identify which events are likely 1 Mile):
      GET https://api.runsignup.com/rest/race/:race_id

Auth:
  RUNSIGNUP_API_KEY, RUNSIGNUP_API_SECRET

Output columns (compatible with your 5K schema):
...
Notes:
  - Heuristics detect a 1 Mile event:
      - event_name contains "1 mile" / "mile" / "1mi" (w/ safety checks)
      - OR distance close to 1.0 miles
      - OR distance close to 1.609 km
  - Split mapping attempts to find Mile 1 only (but file keeps mile2/3 columns as blanks).
"""
...
def looks_like_1mile_event(event: Dict[str, Any]) -> bool:
    ...
    # avoid obvious kids fun runs
    if any(bad in name for bad in ["kid", "kids", "children", "tot", "toddler"]):
        return False

    # Name/type checks (guard against "5K" etc.)
    # We accept "1 mile", "one mile", "1mi", "mile run" (but exclude "mile 1" split text)
    if ("1 mile" in name) or ("one mile" in name) or re.search(r"\b1\s*mi\b", name):
        return True
    if event_type in {"1 mile", "1mile", "mile"} or ("1mile" in event_type) or ("1 mile" in event_type):
        return True

    # Distance check: near 1.0 miles (or 1.609 km)
    miles = _distance_to_miles(dist, unit)
    if miles is not None and abs(miles - 1.0) <= 0.08:
        return True
...
def scrape(
    ...
    while seen < max_result_sets:
        ...
        for rs in result_sets:
            ...
            if not event or not looks_like_1mile_event(event):
                if verbose:
                    en = extract_event_name(event)
                    print(f"[SKIP] Not 1 Mile: race_id={race_id} event_id={event_id} event='{en}' race='{race_name}'",
                          file=sys.stderr)
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape RunSignup 1 Mile results with Mile 1 split into a CSV.")
    ...
    parser.add_argument("--require-all-miles-present", action="store_true",
                        help="Keep only rows that contain required miles for this race distance (here: Mile 1).")
    parser.add_argument("--out", type=str, default="runsignup_1mile_splits.csv")
