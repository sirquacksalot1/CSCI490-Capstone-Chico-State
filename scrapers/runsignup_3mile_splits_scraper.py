"""
runsignup_3mile_splits_scraper.py
...
Same endpoints/auth/output schema as your 5K scraper.
"""
...
def looks_like_3mile_event(event: Dict[str, Any]) -> bool:
    ...
    if ("3 mile" in name) or ("three mile" in name) or re.search(r"\b3\s*mi\b", name):
        return True
    if event_type in {"3 mile", "3mile"} or ("3mile" in event_type) or ("3 mile" in event_type):
        return True

    miles = _distance_to_miles(dist, unit)
    if miles is not None and abs(miles - 3.0) <= 0.15:
        return True
...
def scrape(
    ...
    while seen < max_result_sets:
        ...
        for rs in result_sets:
            ...
            if not event or not looks_like_3mile_event(event):
                if verbose:
                    en = extract_event_name(event)
                    print(f"[SKIP] Not 3 Mile: race_id={race_id} event_id={event_id} event='{en}' race='{race_name}'",
                          file=sys.stderr)
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape RunSignup 3 Mile results with Mile 1/2/3 splits into a CSV.")
    ...
    parser.add_argument("--require-all-miles-present", action="store_true",
                        help="Keep only rows that contain required miles for this race distance (here: Mile 1, 2, 3).")
    parser.add_argument("--out", type=str, default="runsignup_3mile_splits.csv")
